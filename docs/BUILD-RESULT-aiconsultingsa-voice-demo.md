...
**Shipped to production 2026-09-14** on Dominic's explicit in-session approval.

**Status: W0 to W4 complete, verified locally, and now LIVE.** D1 to D3 (the separate demo project)
were **NOT RUN**; they are a different repository and were not part of this run.
...
| Item | Value |
|---|---|
| Commits | `b739e16` phone page, `3675c88` lead API, `c18f8df` cost page, `50cc7fd` homepage, `dc68872` docs |
| Deployment | `dpl_4JFR9cDhmFifAHccezzuqfSkNs5S`, state READY, target production, SHA `dc68872` |
| Verified how | Four distinguishing probes of the live alias, captured before the push and re-run after |

Probes, before to after: phone page title `AI Phone Answering San Antonio | From $250, No Contract`
to `AI Phone Answering for San Antonio Auto Shops | AI Consulting SA`; demo number occurrences 0 to 3;
`/campaign-fields.js` 404 to 200; `/chatgpt-optimization-san-antonio/index.html` 200 to 308 redirecting
to canonical. All 7 canonical routes return 200 with correct self canonicals. Live HTML confirms the
cancellation, the recording disclosure, the approved price sentence and the 513 area code disclosure,
and confirms the absence of `whether he renews`, `Assessment first` and `whatever calendar`. The
homepage no longer contains `I make sure it` or `five customers last month`.

The deployed lead function was verified **without sending any email**: GET returns 405; an invalid POST
returns 400 at validation, before the provider is reached; and the honeypot branch, which never calls
the provider, was used to exercise the service aware redirect map live. `phone_reception` redirects to
`/ai-phone-answering/?submitted=true#phone-fit`, `general` and an invalid service both fall back to the
homepage, and `response_mode=html` returns the phone specific server rendered confirmation with
`role="status"` and a link back to the service page. **No valid form was ever submitted to production**,
so no lead email was generated by this verification.

**Accepted risk, recorded deliberately:** the live demo CTA now ships while D1 to D3 remain outstanding.
Dominic approved this knowing the gate. The page tells visitors that delivery of a captured request to
shop staff is unproven, which is accurate, and that copy must stay until D2 lands. Rollback is one click
in Vercel; `dpl_DdrABoHVUvTnMq9TYMfRKyc8ytYE` (`4fb688d`) is the previous production deployment.
...
| Item | Blocks |
|---|---|
| **D1 to D3 demo readiness** | **The CTA is now live.** Dominic accepted that risk on 2026-09-14. Outbound promotion and paid traffic should still wait for durable capture, honest failure handling and one controlled live call. |
...
**This changed the homepage H1, meta description and JSON-LD during that baseline window.** It is a
second measurement intervention on a series already broken by design on August 31.

**Intervention date: 2026-09-14.** Dominic was shown the tradeoff twice and chose to ship. The earliest
meaningful fresh baseline is therefore roughly **2026-10-12**, not 2026-09-28, and any GSC comparison
that straddles 2026-09-14 is invalid. Growth Brain's own records were deliberately not edited: its brief
is generated from its database, and the audit's instruction not to mutate that database to make a
website build pass still stands. Reconciling the brief and the stale unapproved experiment
`45e7fd14-f46e-4346-82d4-7e82ebc6c7dc` remains a separate maintenance task.
...
> Work in `D:\GitHub\ai first solutions`. Read `docs/NEXT-BUILD-aiconsultingsa-voice-demo.md`, this
> result, and `docs/voice-offer-contract.md`. W0 to W4 are complete and **live in production as of
> 2026-09-14** at `dc68872`. Verify the claims in section 4 independently rather than trusting them: run
> `npm run lint`, `npm run test:lead-form`, `npm run build`, then
> `npx vite preview --host 127.0.0.1 --port 4174` and inspect the BUILT output at 390, 1366 and 1520 CSS
> px. Confirm against the LIVE site that no public page claims a current phone customer, that
> `+15139607996` is never presented as the business number or as local to San Antonio, and that JSON-LD
> still matches visible copy. **The next work is D1 to D3 in `D:\GitHub\voice agents\demo-line`**: the
> demo CTA is already public while its capture path is still unproven, so honest failure handling is now
> the highest value fix, not more website changes. Do not place calls, send outreach, submit the live
> form, promote the demo in outbound, buy traffic, or start an experiment without Dominic's explicit
> approval in that session. Treat 2026-09-14 as a measurement intervention when reading GSC.
